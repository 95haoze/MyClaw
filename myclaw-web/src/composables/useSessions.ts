...
import {
    clearSession as clearRemoteSession,
...
export function useSessions(options: UseSessionsOptions) {
    ...
    async function load(): Promise<boolean> {
        ...
        try {
            ...
            sessions.value = remote.map(session => ({
                id: session.id,
                title: session.title,
                messages: [],
    ...
    function remove(id: string) {
        dialog.value = {
            ...
            targetId: id,
            title: '删除会话',
            description: '该会话及其全部消息将被永久删除，此操作无法撤销。',
            defaultValue: '',
            inputLabel: '',
            confirmText: '删除',
            danger: true
    ...
    function clearCurrent() {
        ...
        dialog.value = {
            ...
            title: '清空会话',
            description: '当前会话中的全部消息将被删除，但会话本身会保留。',
            defaultValue: '',
            inputLabel: '',
            confirmText: '清空',
            danger: true
    ...
    async function confirmDialog(value: string) {
        ...
        try {
            if (item.action === 'rename') {
                ...
                await renameRemoteSession(target.id, value.trim());
