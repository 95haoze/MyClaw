...
export interface UseChatOptions {
    ...
    permissionMode: Ref<'read-only' | 'workspace-write' | 'full-access'>
...
export function useChat(options: UseChatOptions) {
    ...
    async function send(text: string = draft.value, branchIndex?: number): Promise<void> {
        ...
        if (branchIndex !== undefined) {
            ...
            if (!branchMessage?.id) {
                options.reportError('消息尚未完成保存，请稍后再试');
                return
            }
            try {
                await truncateSessionMessages(session.id, branchMessage.id)
            } catch (cause) {
        ...
        try {
            const result = await streamChat({
                ...
                workingDirectory: options.workingDirectory.value.trim() || '.',
    ...
    function addFiles(files: FileList | File[]) {
        for (const file of Array.from(files)) {
            if (file.size > 10 * 1024 * 1024) {
                options.reportError(`${file.name} 超过 10 MB`);
                continue
            }
            if (attachments.value.length >= 5) {
                options.reportError('每次最多添加 5 个附件');
                break
            }
