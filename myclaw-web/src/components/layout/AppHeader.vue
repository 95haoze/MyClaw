...
<template>
  <header class="topbar">
    <div class="crumb">
      <Button class="menu-trigger" variant="ghost" size="icon" aria-label="打开导航" @click="emit('toggleMenu')">
        <Menu :size="17"/>
      </Button>
    ...
    <div class="side">
      <Button v-if="hasMessages" variant="ghost" size="icon" :disabled="busy" aria-label="清空当前会话"
              @click="emit('clear')">
        <Eraser :size="16"/>
      </Button>
      <DropdownMenuRoot>
        ...
        <DropdownMenuPortal>
          <DropdownMenuContent class="account-menu" :side-offset="8" align="end">
            <div class="identity"><strong>{{ currentUser.displayName }}</strong><span>{{ currentUser.email }}</span>
            </div>
            <DropdownMenuSeparator class="menu-separator"/>
            <DropdownMenuItem class="menu-item" @select="emit('openSettings')">
              <Settings2 :size="15"/>
              连接设置
            </DropdownMenuItem>
            <DropdownMenuItem class="menu-item" @select="emit('toggleTheme')">
              <component :is="isDark ? Sun : Moon" :size="15"/>
              {{ isDark ? '浅色主题' : '深色主题' }}
            </DropdownMenuItem>
            <DropdownMenuSeparator class="menu-separator"/>
            <DropdownMenuItem class="menu-item danger" @select="emit('logout')">
              <LogOut :size="15"/>
              退出登录
            </DropdownMenuItem>
          </DropdownMenuContent>
