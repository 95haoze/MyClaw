...
<template>
  <main class="auth">
    <section class="auth-card">
      ...
      <div class="heading"><h1>{{ creating ? '创建工作空间' : '欢迎回来' }}</h1>
        <p>{{ creating ? '注册后开始使用你的 Agent 工作台' : '登录以继续你的对话' }}</p></div>
      <form @submit.prevent="submit">
        ...
        <label><span>密码</span><Input v-model="password" type="password" placeholder="至少 8 位" :minlength="8"
                                       required/></label>
        <p v-if="error" class="error">{{ error }}</p>
        <Button type="submit" class="submit" :disabled="busy">
          <LoaderCircle v-if="busy" class="spin" :size="15"/>
          {{ busy ? '请稍候' : creating ? '注册并登录' : '登录' }}
        </Button>
      </form>
      <Button variant="ghost" class="switch" @click="creating = !creating">{{
          creating ? '已有账户？登录' : '没有账户？注册'
        }}
      </Button>
    </section>
  </main>
